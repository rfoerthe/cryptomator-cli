...
// Vendored-fork additions (see ../README-VENDORED.md). The workspace gate is
// `cargo clippy --workspace --all-targets -- -D warnings`, which is stricter than what upstream
// fuser runs. These allows silence pre-existing upstream findings so the gate stays meaningful
// for our own code; rewriting upstream code would only make future rebases harder.
//
// `clippy::io_other_error`: three `io::Error::new(ErrorKind::Other, _)` calls in `src/notify.rs`,
// `src/passthrough.rs` and `src/session.rs`.
...
pub use crate::mnt::mount_options::KernelAbi;
