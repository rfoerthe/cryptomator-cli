...
// ---------------------------------------------------------------------------------------------
// Linux-layout twins (vendored patch, see ../../README-VENDORED.md)
//
// macFUSE adds Darwin-only fields to a handful of wire structs, and the definitions above encode
// them unconditionally when building for macOS. A peer that speaks the Linux protocol (FUSE-T)
// expects the structs without those fields. The twins below reproduce the Linux layouts so that a
// session configured with `KernelAbi::Linux` can talk to such a peer. They only exist on macOS;
// everywhere else the definitions above are already the Linux ones.
// ---------------------------------------------------------------------------------------------

/// `fuse_attr` without the Darwin-only `crtime`, `crtimensec` and `flags` fields (88 bytes).
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, IntoBytes, Clone, Copy, KnownLayout, Immutable)]
...
#[cfg(target_os = "macos")]
impl From<&fuse_attr> for fuse_attr_linux {
    fn from(a: &fuse_attr) -> Self {
        // `crtime`/`crtimensec` have no place in the Linux layout and are dropped. `flags` exists
        // in both, but means `chflags(2)` on Darwin and `FUSE_ATTR_*` on Linux, so it is not
        // carried over.
        Self {
            ino: a.ino,
            size: a.size,
...
/// `fuse_entry_out` carrying a Linux-layout `fuse_attr`.
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_entry_out_linux {
    pub(crate) nodeid: u64,
    pub(crate) generation: u64,
    pub(crate) entry_valid: u64,
    pub(crate) attr_valid: u64,
    pub(crate) entry_valid_nsec: u32,
    pub(crate) attr_valid_nsec: u32,
    pub(crate) attr: fuse_attr_linux,
}

#[cfg(target_os = "macos")]
impl From<&fuse_entry_out> for fuse_entry_out_linux {
    fn from(e: &fuse_entry_out) -> Self {
        Self {
            nodeid: e.nodeid,
            generation: e.generation,
            entry_valid: e.entry_valid,
            attr_valid: e.attr_valid,
            entry_valid_nsec: e.entry_valid_nsec,
            attr_valid_nsec: e.attr_valid_nsec,
            attr: (&e.attr).into(),
        }
    }
}

/// `fuse_attr_out` carrying a Linux-layout `fuse_attr`.
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_attr_out_linux {
    pub(crate) attr_valid: u64,
    pub(crate) attr_valid_nsec: u32,
    pub(crate) dummy: u32,
    pub(crate) attr: fuse_attr_linux,
...
#[cfg(target_os = "macos")]
impl From<&fuse_attr_out> for fuse_attr_out_linux {
    fn from(a: &fuse_attr_out) -> Self {
        Self {
            attr_valid: a.attr_valid,
            attr_valid_nsec: a.attr_valid_nsec,
            dummy: a.dummy,
            attr: (&a.attr).into(),
        }
    }
...
/// `fuse_create_out` carrying a Linux-layout `fuse_entry_out`.
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_create_out_linux(pub(crate) fuse_entry_out_linux, pub(crate) fuse_open_out);

#[cfg(target_os = "macos")]
impl From<&fuse_create_out> for fuse_create_out_linux {
    fn from(c: &fuse_create_out) -> Self {
        Self(
            (&c.0).into(),
            fuse_open_out {
                fh: c.1.fh,
                open_flags: c.1.open_flags,
                backing_id: c.1.backing_id,
            },
        )
    }
}

/// `fuse_direntplus` carrying a Linux-layout `fuse_entry_out`.
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_direntplus_linux {
    pub(crate) entry_out: fuse_entry_out_linux,
    pub(crate) dirent: fuse_dirent,
}

#[cfg(target_os = "macos")]
impl From<&fuse_direntplus> for fuse_direntplus_linux {
    fn from(d: &fuse_direntplus) -> Self {
        Self {
            entry_out: (&d.entry_out).into(),
            dirent: fuse_dirent {
                ino: d.dirent.ino,
                off: d.dirent.off,
                namelen: d.dirent.namelen,
                typ: d.dirent.typ,
            },
        }
    }
...
/// `fuse_setattr_in` without the seven Darwin-only trailing fields (88 bytes).
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, FromBytes, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_setattr_in_linux {
    pub(crate) valid: u32,
    pub(crate) padding: u32,
    pub(crate) fh: u64,
    pub(crate) size: u64,
...
#[cfg(target_os = "macos")]
impl From<&fuse_setattr_in_linux> for fuse_setattr_in {
    fn from(s: &fuse_setattr_in_linux) -> Self {
        // A Linux peer never sets the Darwin-only FATTR_* bits, so the corresponding fields are
        // left at zero and the accessors on `SetAttr` report them as absent.
        Self {
            valid: s.valid,
            padding: s.padding,
            fh: s.fh,
            ...
            mtime: s.mtime,
            ctime: s.ctime,
            atimensec: s.atimensec,
            mtimensec: s.mtimensec,
            ctimensec: s.ctimensec,
            mode: s.mode,
            unused4: s.unused4,
            uid: s.uid,
            gid: s.gid,
...
/// `fuse_getxattr_in` without the Darwin-only `position` field (8 bytes).
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, FromBytes, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_getxattr_in_linux {
    pub(crate) size: u32,
    pub(crate) padding: u32,
}

#[cfg(target_os = "macos")]
impl From<&fuse_getxattr_in_linux> for fuse_getxattr_in {
    fn from(g: &fuse_getxattr_in_linux) -> Self {
        Self {
            size: g.size,
            padding: g.padding,
            position: 0,
...
/// `fuse_setxattr_in` without the Darwin-only `position` field (8 bytes).
#[cfg(target_os = "macos")]
#[repr(C)]
#[derive(Debug, FromBytes, IntoBytes, KnownLayout, Immutable)]
pub(crate) struct fuse_setxattr_in_linux {
    pub(crate) size: u32,
    pub(crate) flags: i32,
}

#[cfg(target_os = "macos")]
impl From<&fuse_setxattr_in_linux> for fuse_setxattr_in {
    fn from(s: &fuse_setxattr_in_linux) -> Self {
        Self {
            size: s.size,
            flags: s.flags,
            position: 0,
