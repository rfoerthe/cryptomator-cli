...
mod op {
    ...
    /// Decode `fuse_setattr_in`, honouring `abi`.
    ///
    /// A Linux peer sends the struct without the seven Darwin-only trailing fields, so on macOS
    /// the shorter twin is decoded and widened; those fields end up zeroed, which the `SetAttr`
    /// accessors report as absent.
    fn fetch_setattr_in<'a>(
        data: &mut ArgumentIterator<'a>,
        abi: KernelAbi,
    ) -> Option<ArgRef<'a, fuse_setattr_in>> {
        #[cfg(target_os = "macos")]
        if abi == KernelAbi::Linux {
            let arg: &fuse_setattr_in_linux = data.fetch()?;
            return Some(ArgRef::Owned(Box::new(arg.into())));
        }
        #[cfg(not(target_os = "macos"))]
        let _ = abi;
        Some(ArgRef::Borrowed(data.fetch()?))
    }

    /// Decode `fuse_getxattr_in` (used by both `GETXATTR` and `LISTXATTR`), honouring `abi`.
    /// A Linux peer omits the Darwin-only `position` field, which is then reported as 0.
    fn fetch_getxattr_in<'a>(
        data: &mut ArgumentIterator<'a>,
        abi: KernelAbi,
    ) -> Option<ArgRef<'a, fuse_getxattr_in>> {
        #[cfg(target_os = "macos")]
        if abi == KernelAbi::Linux {
            let arg: &fuse_getxattr_in_linux = data.fetch()?;
            return Some(ArgRef::Owned(Box::new(arg.into())));
        }
        #[cfg(not(target_os = "macos"))]
        let _ = abi;
    ...
    /// Decode `fuse_setxattr_in`, honouring `abi`.
    /// A Linux peer omits the Darwin-only `position` field, which is then reported as 0.
    fn fetch_setxattr_in<'a>(
        data: &mut ArgumentIterator<'a>,
        abi: KernelAbi,
    ) -> Option<ArgRef<'a, fuse_setxattr_in>> {
        #[cfg(target_os = "macos")]
        if abi == KernelAbi::Linux {
            let arg: &fuse_setxattr_in_linux = data.fetch()?;
            return Some(ArgRef::Owned(Box::new(arg.into())));
        }
        #[cfg(not(target_os = "macos"))]
        let _ = abi;
    ...
    pub(crate) fn parse<'a>(
        ...
        Some(match opcode {
            ...
            fuse_opcode::FUSE_SETATTR => Operation::SetAttr(SetAttr {
                header,
                arg: fetch_setattr_in(&mut data, abi)?,
            }),
            ...
            fuse_opcode::FUSE_SETXATTR => Operation::SetXAttr({
                let out = SetXAttr {
                    header,
                    arg: fetch_setxattr_in(&mut data, abi)?,
                    name: data.fetch_str()?,
            ...
            fuse_opcode::FUSE_GETXATTR => Operation::GetXAttr(GetXAttr {
                header,
                arg: fetch_getxattr_in(&mut data, abi)?,
                name: data.fetch_str()?,
            }),
...
/// A parsed request argument.
///
/// Normally this is a zero-copy view into the request buffer. When the peer speaks a different
/// wire layout than the target platform ([`KernelAbi::Linux`] on macOS) the shorter struct is
/// decoded and widened into an owned one, because there is nothing in the buffer to point at.
#[derive(Debug)]
pub(crate) enum ArgRef<'a, T> {
    Borrowed(&'a T),
    #[cfg(target_os = "macos")]
    Owned(Box<T>),
}

impl<T> std::ops::Deref for ArgRef<'_, T> {
    type Target = T;
...
pub(crate) struct AnyRequest<'a> {
    header: &'a fuse_in_header,
    data: &'a [u8],
    /// Wire layout the peer speaks; see [`KernelAbi`].
    abi: KernelAbi,
}
...
impl<'a> AnyRequest<'a> {
    ...
    pub(crate) fn operation(&self) -> Result<Operation<'a>, RequestError> {
        ...
        op::parse(self.header, &opcode, self.data, self.abi).ok_or(RequestError::InsufficientData)
    }
...
impl<'a> AnyRequest<'a> {
    /// Parse a raw packet as sent by the kernel driver into typed data, decoding arguments in the
    /// layout `abi` selects. Every request always begins with a `fuse_in_header` struct followed
    /// by arguments depending on the opcode.
    pub(crate) fn parse(data: &'a [u8], abi: KernelAbi) -> Result<Self, RequestError> {
...
impl<'a> TryFrom<&'a [u8]> for AnyRequest<'a> {
    ...
    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        Self::parse(data, KernelAbi::Native)
    }
}
...
/// Request-parsing tests for `KernelAbi::Linux` (vendored patch, see ../../README-VENDORED.md).
///
/// macOS-only: on every other target the native request layout already is the Linux one.
#[cfg(all(test, target_os = "macos"))]
mod abi_test {
    use zerocopy::IntoBytes;

    use crate::KernelAbi;
    use crate::ll::fuse_abi::fuse_getxattr_in;
    use crate::ll::fuse_abi::fuse_getxattr_in_linux;
    use crate::ll::fuse_abi::fuse_setattr_in;
    use crate::ll::fuse_abi::fuse_setattr_in_linux;
    use crate::ll::fuse_abi::fuse_setxattr_in;
    use crate::ll::fuse_abi::fuse_setxattr_in_linux;
    use crate::ll::request::*;
    use crate::ll::test::AlignedData;

    const HEADER: usize = size_of::<fuse_in_header>();

    /// Build a request buffer: a `fuse_in_header` for `opcode`, then `arg`, then `tail`.
    fn request<const N: usize>(opcode: u32, arg: &[u8], tail: &[u8]) -> AlignedData<[u8; N]> {
        let mut buf = AlignedData([0u8; N]);
        // fuse_in_header: len, opcode, unique, nodeid, uid, gid, pid, padding
        buf[0..4].copy_from_slice(&(N as u32).to_ne_bytes());
        buf[4..8].copy_from_slice(&opcode.to_ne_bytes());
        buf[8..16].copy_from_slice(&0xdead_beef_u64.to_ne_bytes());
        buf[16..24].copy_from_slice(&1u64.to_ne_bytes());
        buf[24..28].copy_from_slice(&501u32.to_ne_bytes());
        buf[28..32].copy_from_slice(&20u32.to_ne_bytes());
        buf[32..36].copy_from_slice(&42u32.to_ne_bytes());
        buf[HEADER..HEADER + arg.len()].copy_from_slice(arg);
        buf[HEADER + arg.len()..].copy_from_slice(tail);
        buf
    }

    #[test]
    fn setattr_parses_the_88_byte_linux_struct() {
        // FATTR_MODE | FATTR_UID | FATTR_GID | FATTR_SIZE
        let arg = fuse_setattr_in_linux {
            valid: 0x01 | 0x02 | 0x04 | 0x08,
            padding: 0,
            fh: 0,
            size: 0x1234,
            lock_owner: 0,
        ...
        assert_eq!(size_of::<fuse_setattr_in_linux>(), 88);
        assert_eq!(size_of::<fuse_setattr_in>(), 128);

        let buf = request::<{ HEADER + 88 }>(4, arg.as_bytes(), &[]);
        // The native parser wants 128 bytes of argument and must reject the short request.
        assert!(matches!(
            AnyRequest::parse(&buf[..], KernelAbi::Native)
                .expect("header")
                .operation(),
            Err(RequestError::InsufficientData)
        ));

        let req = AnyRequest::parse(&buf[..], KernelAbi::Linux).expect("header");
        let Ok(Operation::SetAttr(x)) = req.operation() else {
            panic!("expected SetAttr");
        };
        assert_eq!(x.mode(), Some(0o100644));
        assert_eq!(x.uid(), Some(501));
        assert_eq!(x.gid(), Some(20));
        assert_eq!(x.size(), Some(0x1234));
        // The Darwin-only fields are absent, not garbage.
    ...
    #[test]
    fn getxattr_parses_the_8_byte_linux_struct() {
        let arg = fuse_getxattr_in_linux {
            size: 64,
            padding: 0,
        };
        assert_eq!(size_of::<fuse_getxattr_in_linux>(), 8);
        assert_eq!(size_of::<fuse_getxattr_in>(), 16);

        let buf = request::<{ HEADER + 8 + 8 }>(22, arg.as_bytes(), b"user.a\0\0");
        let req = AnyRequest::parse(&buf[..], KernelAbi::Linux).expect("header");
        let Ok(Operation::GetXAttr(x)) = req.operation() else {
            panic!("expected GetXAttr");
        };
        assert_eq!(x.size_u32(), 64);
        assert_eq!(x.name(), "user.a");
    }

    #[test]
    fn setxattr_parses_the_8_byte_linux_struct() {
        let arg = fuse_setxattr_in_linux { size: 2, flags: 0 };
        assert_eq!(size_of::<fuse_setxattr_in_linux>(), 8);
        assert_eq!(size_of::<fuse_setxattr_in>(), 16);

        let buf = request::<{ HEADER + 8 + 7 + 2 }>(21, arg.as_bytes(), b"user.a\0hi");
        let req = AnyRequest::parse(&buf[..], KernelAbi::Linux).expect("header");
        let Ok(Operation::SetXAttr(x)) = req.operation() else {
            panic!("expected SetXAttr");
        };
        assert_eq!(x.name(), "user.a");
        assert_eq!(x.value(), b"hi");
        // `position` does not exist in the Linux layout.
