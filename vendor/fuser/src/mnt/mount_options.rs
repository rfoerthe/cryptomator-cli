...
/// Which kernel struct layouts the peer on the other end of the FUSE fd expects.
///
/// The FUSE wire structs are not identical on all platforms: macFUSE adds Darwin-only fields to
/// `fuse_attr`, `fuse_setattr_in`, `fuse_getxattr_in` and `fuse_setxattr_in`, and fuser encodes
/// those Darwin layouts whenever it is built for macOS. FUSE-T, a userspace NFS server that speaks
/// the same protocol over a plain fd, instead expects the Linux layouts. Selecting
/// [`KernelAbi::Linux`] makes the session encode replies and decode requests using the Linux
/// layouts even on macOS.
///
/// On every non-macOS target the two variants are byte-for-byte identical.
#[derive(Debug, Clone, Copy, Default, Eq, PartialEq, Hash)]
pub enum KernelAbi {
    /// The layouts native to the target platform (macFUSE layouts on macOS).
    #[default]
    Native,
    /// The Linux layouts, on every platform.
    Linux,
}
...
pub struct Config {
    ...
    pub clone_fd: bool,
    /// Which kernel struct layouts the peer expects. Defaults to [`KernelAbi::Native`], the layout
    /// of the target platform and the only correct choice for a real kernel driver.
    pub abi: KernelAbi,
}
...
mod test {
    ...
    /// The ABI switch must stay opt-in: a default `Config` talks to a real kernel driver, which
    /// always uses the layouts native to the platform.
    #[test]
    fn default_abi_is_native() {
        assert_eq!(Config::default().abi, KernelAbi::Native);
        assert_eq!(KernelAbi::default(), KernelAbi::Native);
    }
